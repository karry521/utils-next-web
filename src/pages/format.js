...
const Format = () => {
    ...
    // 监听是否为24小时制
    useEffect(() => {
        const regex = /^\s*(?:(\d{1,2})\s*[h时]?\s*(\d{1,2})\s*[m分]?\s*(\d{1,2})\s*[s秒]?)|(?:^(\d{1,2})\s*[m分]\s*(\d{1,2})\s*[s秒]?)|(?:^(\d{1,2})\s*:\s*(\d{1,2}))|(?:^(\d{1,2})\s*:\s*(\d{1,2})\s*:\s*(\d{1,2}))\s*$/
    ...
    const handleJsonData = () => {
        ...
        const keyArr = Array.from(
            new Set(
                jsonData.split(/(?:"([^"]+)"|([a-zA-Z_][a-zA-Z0-9_]*))\s*:/).filter((item, index) => (index % 3 === 1 || index % 3 === 2) && item !== undefined)
            )
        )
        ...
        nowJsonData = nowJsonData.trim().replaceAll(/(?<!")\b([a-zA-Z0-9_]+)\b(?=\s*:)/g, '"$1"').replaceAll(/:\s*([a-zA-Z_]+)(?=\s*[},\]])/g, ': "$1"').replaceAll(/'(.*?)'/g, '"$1"').replaceAll(/`(.*?)`/g, '"$1"')
    ...
    const processDynamicString = (originalString, keyName, generateTimestamp) => {
        ...
        const replaceValues = (str, timestamps) => {
            let index = 0
            return str.replace(
                new RegExp(`(${keyName}\\s*:\\s*)(\\d+|'[^']*'|"[^"]*"|\`[^\`]*\`)`, 'g'),
                (_, prefix) => `${prefix}${timestamps[index++]}`
            )
        }
    ...
    const generateTimestamp = length => {
        ...
        let times = []
        const {
            type, isDate, timeType, startDate, startTime, endDate, endTime, startTime2, endTime2,
            sort, sortOrder, fixedValue, totalFlag, totalTime, totalUnit, fixedNull, keySymbol
        } = settingParams
        ...
        <div className='w-full flex justify-center'>
            <div className='w-full max-w-[1440px] p-4 mt-12 shadow-[0px_8px_22px_0px_#0000001F] rounded-[8px]'>
                ...
                <div className="flex justify-start gap-6 mt-8">
                    <button className='w-auto px-6 h-[50px] bg-sky-500 text-white rounded-[8px]' onClick={() => copyTimestamp()}>生成并复制当前时间点时间戳</button>
                </div>
                ...
                <div className='flex justify-start items-center mt-4 ml-[21%]'>
                    ...
                    <input type='time' step='1' className='w-[121px] [border:1px_skyblue_solid] rounded-[8px] pl-4 ml-2 mr-2' disabled={timeEnable} value={timeValue} onChange={(e) => {
                        clearInterval(time)
                        setTimeValue(e.target.value)
                    }} />

                    <button className='w-auto ml-4 px-2 h-[32px] bg-sky-500 text-[14px] text-white rounded-[8px]' onClick={() => copyTimestamp(new Date(dateValue + ' ' + timeValue).getTime())}>复制左侧时间点的时间戳</button>
                ...
                <div className='flex justify-start items-center mt-4 ml-[21%]'>
                    ...
                    <input type='text' id='textValue' className='w-[268px] [border:1px_skyblue_solid] rounded-[8px] pl-2 outline-none' placeholder="在这里输入要转换的时间" value={textValue} onChange={(e) => setTextValue(e.target.value)} />
                ...
                <div className='flex justify-start items-center mt-4 ml-[21%]'>
                    ...
                    <input type='text' id='timestampValue' className='w-[268px] [border:1px_skyblue_solid] rounded-[8px] pl-2 outline-none' placeholder="在这里输入要转换的时间戳" value={timestampValue} onChange={(e) => setTimestampValue(e.target.value)} />

                    <div
                        className='w-[72px] flex justify-center relative cursor-pointer select-none ml-4'
                        onMouseOver={() => setTimestampFlag(true)}
                        onMouseOut={() => setTimestampFlag(false)}
                    >
                        {timestampUnit === 'ms' ? '毫秒' : '秒'}
                        <Image src='/arrow.svg' alt='arrow' width={20} height={20} className={`transition-[rotate.2s_linear] ${timestampFlag ? 'rotate-[180deg]' : 'rotate[360deg]'} absolute top-[50%] -translate-y-[50%] right-0`} />
                        ...
                        <ul className={`absolute right-[8%] top-[100%] text-white text-center bg-sky-500 px-2 overflow-hidden transition-[max-height_.2s_linear] ${timestampFlag ? 'max-h-[68px]' : 'max-h-0'} z-[52]`}>
                            <li className='py-1 hover:text-sky-200' onClick={() => setTimestampUnit('ms')}>毫秒</li>
                            <li className='py-1 hover:text-sky-200' onClick={() => setTimestampUnit('s')}>秒</li>
                        </ul>
                    </div>

                    <div
                        className='w-[225px] flex justify-center relative cursor-pointer select-none ml-4'
                        onMouseOver={() => setTimestampFlag2(true)}
                        onMouseOut={() => setTimestampFlag2(false)}
                    >
                        {timestampType}
                        <Image src='/arrow.svg' alt='arrow' width={20} height={20} className={`transition-[rotate.2s_linear] ${timestampFlag2 ? 'rotate-[180deg]' : 'rotate[360deg]'} absolute top-[50%] -translate-y-[50%] right-0`} />
                        ...
                        <ul className={`absolute right-[8%] top-[100%] text-white text-center bg-sky-500 px-2 overflow-hidden transition-[max-height_.2s_linear] ${timestampFlag2 ? 'max-h-[168px]' : 'max-h-0'} z-[52]`}>
                            <li className='py-1 hover:text-sky-200' onClick={() => setTimestampType('YYYY-MM-DD HH:mm:ss')}>YYYY-MM-DD HH:mm:ss</li>
                            <li className='py-1 hover:text-sky-200' onClick={() => setTimestampType('YYYY-MM-DD')}>YYYY-MM-DD</li>
                            <li className='py-1 hover:text-sky-200' onClick={() => setTimestampType('HH:mm:ss')}>HH:mm:ss</li>
                            <li className='py-1 hover:text-sky-200' onClick={() => setTimestampType('HH:mm')}>HH:mm</li>
                            <li className='py-1 hover:text-sky-200' onClick={() => setTimestampType('mm:ss')}>mm:ss</li>
                        </ul>
                    </div>
                ...
                <div className='flex justify-start items-center mt-4 ml-[21%]'>
                    ...
                    <div className='flex flex-col gap-2'>
                        <input type='text' id='startDateTime' className='w-[268px] [border:1px_skyblue_solid] rounded-[8px] pl-2 outline-none' placeholder="在这里输入开始日期时间或时间戳" value={startDateTime} onChange={(e) => setStartDateTime(e.target.value)} />
                        <input type='text' id='endDateTime' className='w-[268px] [border:1px_skyblue_solid] rounded-[8px] pl-2 outline-none' placeholder="在这里输入结束日期时间或时间戳" value={endDateTime} onChange={(e) => setEndDateTime(e.target.value)} />
                    </div>

                    <div
                        className='w-[138px] flex justify-center relative cursor-pointer select-none'
                        onMouseOver={() => setDateTimeFlag(true)}
                        onMouseOut={() => setDateTimeFlag(false)}
                    >
                        {dateTimeType === 0 ? '每次结果累计' : '结果不累计'}
                        <Image src='/arrow.svg' alt='arrow' width={20} height={20} className={`transition-[rotate.2s_linear] ${dateTimeFlag ? 'rotate-[180deg]' : 'rotate[360deg]'} ease-linear absolute top-[50%] -translate-y-[50%] right-0`} />
                        ...
                        <ul className={`absolute right-[8%] top-[100%] text-white text-center bg-sky-500 px-2 overflow-hidden transition-[max-height_.2s_linear] ${dateTimeFlag ? 'max-h-[68px]' : 'max-h-0'} z-[52]`}>
                            <li className='py-1 hover:text-sky-200' onClick={() => setDateTimeType(0)}>每次结果累计</li>
                            <li className='py-1 hover:text-sky-200' onClick={() => setDateTimeType(1)}>结果不累计</li>
                        </ul>
                    </div>

                    <button className='w-auto max-w-[121px] ml-4 px-2 h-[32px] bg-sky-500 text-[14px] text-white rounded-[8px]' onClick={() => calculateDateTime()}>点击计算</button>

                    <div className='flex flex-col gap-2'>
                        <input type='text' className='w-[521px] text-center bg-gray-300 rounded-[6px] ml-6 outline-none' placeholder="计算后的结果显示在这"
                            value={`两者相差: ${dateTimeDiff[0]}年 ${dateTimeDiff[1]}月 ${dateTimeDiff[2]}天 ${dateTimeDiff[3]}时 ${dateTimeDiff[4]}分 ${dateTimeDiff[5]}秒`} readOnly />

                        <input type='text' className='w-[521px] text-center bg-gray-300 rounded-[6px] ml-6 outline-none' placeholder="计算后的结果显示在这"
                            value={`总差: ${dateTimeDiff2[0]}年 / ${dateTimeDiff2[1]}月 / ${dateTimeDiff2[2]}天 / ${dateTimeDiff2[3]}时 / ${dateTimeDiff2[4]}分 / ${dateTimeDiff2[5]}秒`} readOnly />
                    </div>
                </div>

                {/* 批量计算时间差 */}
                <div className='flex justify-start items-center mt-4 ml-[21%]'>
                    <ScrollTextarea id='diffData'
                        className={`w-[408px] min-h-[128px] rounded-[8px] pl-2 pt-2 outline-none placeholder:text-center placeholder:leading-[105px] mt-2  ${diffNull ? 'placeholder:text-red-500 caret-red-500 [border:1px_red_solid]' : '[border:1px_skyblue_solid]'}`}
                        placeholder="请输入包含时间数组的大数组" value={diffData} onChange={e => {
                    ...
                    <div className='flex flex-col gap-2'>
                        <button className='w-auto max-w-[121px] ml-4 px-2 h-[32px] bg-sky-500 text-[14px] text-white rounded-[8px]' onClick={() => {
                            if (diffData.trim() === '') {
                                setDiffNull(true)
                    ...
                    <div className='flex flex-col gap-2'>
                        <input type='text' className='w-[521px] text-center bg-gray-300 rounded-[6px] ml-6 outline-none' placeholder="计算后的结果显示在这"
                            value={`累加相差: ${dateTimeDiffs[0]}年 ${dateTimeDiffs[1]}月 ${dateTimeDiffs[2]}天 ${dateTimeDiffs[3]}时 ${dateTimeDiffs[4]}分 ${dateTimeDiffs[5]}秒`} readOnly />

                        <input type='text' className='w-[521px] text-center bg-gray-300 rounded-[6px] ml-6 outline-none' placeholder="计算后的结果显示在这"
                            value={`累加总差: ${dateTimeDiffs2[0]}年 / ${dateTimeDiffs2[1]}月 / ${dateTimeDiffs2[2]}天 / ${dateTimeDiffs2[3]}时 / ${dateTimeDiffs2[4]}分 / ${dateTimeDiffs2[5]}秒`} readOnly />
                    </div>
                </div>

                {/* 批量插入时间戳or秒数 */}
                <div className='flex justify-start items-center mt-4 ml-[21%]'>
                    <ScrollTextarea id='jsonData'
                        className={`w-[521px] min-h-[213px] rounded-[8px] pl-2 pt-2 outline-none placeholder:text-center placeholder:leading-[202px]  ${jsonNull ? 'placeholder:text-red-500 caret-red-500 [border:1px_red_solid]' : '[border:1px_skyblue_solid]'}`}
                        placeholder="请输入json格式内容(可不包含最外层大括号)" value={jsonData} onChange={e => {
                    ...
                    <div className='flex flex-col gap-4'>
                        ...
                        <div>
                            <button className='w-[128px] mx-4 px-2 h-[32px] bg-sky-500 text-[14px] text-white rounded-[8px]' onClick={() => setShowModal(true)}>点击设置插入规则</button>
                            <span className='text-[12px]'>tips:默认采用近三个月的日期随机生成包含（年月日小时分钟秒）的时间戳</span>
                        </div>
                        <button className='w-[128px] ml-4 px-2 h-[32px] bg-sky-500 text-[14px] text-white rounded-[8px] disabled:text-gray-500' onClick={() => {
                            if (jsonData.trim() === '') {
                                setJsonNull(true)
                        ...
                        <button className='w-[128px] ml-4 px-2 h-[32px] bg-sky-500 text-[14px] text-white rounded-[8px] disabled:text-gray-500' onClick={() => copyTimestamp(jsonData)}>复制json结果</button>
                    </div>
                </div>
            </div>
            ...
            {/* 遮罩层 */}
            <div className={`w-[99vw] h-[100vh] bg-transparent absolute top-0 left-0 ${!showModal ? 'hidden' : ''} z-[52]`} onClick={() => {
            ...
            {/* 设置弹窗 */}
            <div className={`w-[521px] h-[454px] bg-[#FFFFFF] shadow-[0px_0px_22px_0px_#0000002F] rounded-[8px] absolute transition-top duration-150 ease-linear ${!showModal ? '-top-[545px]' : 'top-[6%]'} select-none z-[54]`}>
                ...
                <div className='p-4 flex flex-col gap-3'>
                    ...
                    <div className='flex items-center gap-4'>
                        <span>插入数据类型:</span>
                        <label className='cursor-pointer'>
                            <input type='radio' name='isDate' className='cursor-pointer' checked={settingParams.isDate} onChange={() => setSettingParams(params => ({ ...params, isDate: true }))} />&nbsp;日期(2000-10-03)
                        </label>
                        <label className='cursor-pointer'>
                            <input type='radio' name='isDate' className='cursor-pointer' checked={!settingParams.isDate} onChange={() => setSettingParams(params => ({ ...params, isDate: false }))} />&nbsp;时间(13:14:52)
                        </label>
                    </div>
                    ...
                    <div className={`flex items-center gap-4 ${settingParams.isDate ? 'text-gray-300' : ''}`}>
                        <span>时间格式:</span>
                        <label className='cursor-pointer'>
                            <input type='radio' name='timeType' className='cursor-pointer'
                                disabled={settingParams.isDate} checked={settingParams.timeType === 0} onChange={() => setSettingParams(params => ({ ...params, timeType: 0 }))} />&nbsp;时分秒(13:14:52)
                        </label>
                        <label className='cursor-pointer'>
                            <input type='radio' name='timeType' className='cursor-pointer'
                                disabled={settingParams.isDate} checked={settingParams.timeType === 1} onChange={() => setSettingParams(params => ({ ...params, timeType: 1 }))} />&nbsp;分秒(13:14)
                        </label>
                    </div>
                    ...
                    <div className='flex items-center gap-4'>
                        <span>排序:</span>
                        <label className='cursor-pointer'>
                            <input type='radio' name='sort' className='cursor-pointer' checked={settingParams.sort} onChange={() => setSettingParams(params => ({ ...params, sort: true }))} />&nbsp;启用
                        </label>
                        <label className='cursor-pointer'>
                            <input type='radio' name='sort' className='cursor-pointer' checked={!settingParams.sort} onChange={() => setSettingParams(params => ({ ...params, sort: false }))} />&nbsp;关闭
                        </label>

                        <div className={`flex items-center ml-6 gap-4 ${!settingParams.sort ? 'text-gray-300' : ''}`}>
                            <span>顺序:</span>
                            <label className='cursor-pointer'>
                                <input type='radio' name='sortOrder' className='cursor-pointer'
                                    disabled={!settingParams.sort} checked={settingParams.sortOrder === 'desc'} onChange={() => setSettingParams(params => ({ ...params, sortOrder: 'desc' }))} />&nbsp;降序
                            </label>
                            <label className='cursor-pointer'>
                                <input type='radio' name='sortOrder' className='cursor-pointer'
                                    disabled={!settingParams.sort} checked={settingParams.sortOrder === 'asc'} onChange={() => setSettingParams(params => ({ ...params, sortOrder: 'asc' }))} />&nbsp;升序
                            </label>
                        </div>
                    </div>
                    ...
                    <div className='flex items-center gap-4'>
                        <span>根据区间随机生成:</span>
                        <label className='cursor-pointer'>
                            <input type='radio' name='type' className='cursor-pointer' checked={settingParams.type} onChange={() => setSettingParams(params => ({ ...params, type: true }))} />&nbsp;启用
                        </label>
                        <label className='cursor-pointer'>
                            <input type='radio' name='type' className='cursor-pointer' checked={!settingParams.type} onChange={() => setSettingParams(params => ({ ...params, type: false }))} />&nbsp;关闭
                        </label>
                    </div>
                            ...
                            <label className='cursor-pointer'>
                                开始时间:&nbsp;
                                {
                                    settingParams.isDate ?
                                        <Fragment>
                                            <input type='date' className='w-[128px] [border:1px_skyblue_solid] rounded-[8px] pl-2 mr-2' disabled={!settingParams.type}
                                                value={settingParams.startDate} onChange={(e) => setSettingParams(params => ({ ...params, startDate: e.target.value }))} />
                                            <input type='time' step='1' className='w-[121px] [border:1px_skyblue_solid] rounded-[8px] pl-4 ml-2 mr-2' disabled={!settingParams.type}
                                                value={settingParams.startTime} onChange={(e) => setSettingParams(params => ({ ...params, startTime: e.target.value }))} />
                                        </Fragment>
                                        :
                                        <input type='time' step='1' className='w-[121px] [border:1px_skyblue_solid] rounded-[8px] pl-4 ml-2 mr-2' disabled={!settingParams.type}
                                            value={settingParams.startTime2} onChange={(e) => setSettingParams(params => ({ ...params, startTime2: e.target.value }))} />
                                }
                            </label>
                            <label className='cursor-pointer'>
                                结束时间:&nbsp;
                                {
                                    settingParams.isDate ?
                                        <Fragment>
                                            <input type='date' className='w-[128px] [border:1px_skyblue_solid] rounded-[8px] pl-2 mr-2' disabled={!settingParams.type}
                                                value={settingParams.endDate} onChange={(e) => setSettingParams(params => ({ ...params, endDate: e.target.value }))} />
                                            <input type='time' step='1' className='w-[121px] [border:1px_skyblue_solid] rounded-[8px] pl-4 ml-2 mr-2' disabled={!settingParams.type}
                                                value={settingParams.endTime} onChange={(e) => setSettingParams(params => ({ ...params, endTime: e.target.value }))} />
                                        </Fragment>
                                        :
                                        <input type='time' step='1' className='w-[121px] [border:1px_skyblue_solid] rounded-[8px] pl-4 ml-2 mr-2' disabled={!settingParams.type}
                                            value={settingParams.endTime2} onChange={(e) => setSettingParams(params => ({ ...params, endTime2: e.target.value }))} />
                                }
                            </label>
                        </div>
                    </div>
                    ...
                    <div className={`flex items-start gap-2 ${settingParams.type ? 'text-gray-300' : ''}`}>
                        <span>根据固定值生成:</span>
                        <label className='cursor-pointer'>
                            <input type='text' id='fixedValue'
                                className={`w-[312px] rounded-[8px] cursor-pointer outline-none pl-2 ${!settingParams.type && settingParams.fixedNull ? 'placeholder:text-red-500 caret-red-500 [border:1px_red_solid]' : '[border:1px_skyblue_solid]'}`}
                                placeholder='在这里输入你要插入的固定时间日期'
                                value={settingParams.fixedValue} disabled={settingParams.type} onChange={e => setSettingParams(params => ({ ...params, fixedValue: e.target.value, fixedNull: false }))} />
                        </label>
                    </div>
                    ...
                    <div className={`flex items-start gap-2 ${settingParams.isDate || !settingParams.type ? 'text-gray-300' : ''}`}>
                        <span>限制生成时间总和:</span>
                        <label className='cursor-pointer'>
                            <input type='radio' name='totalFlag' disabled={settingParams.isDate || !settingParams.type}
                                className='cursor-pointer' checked={settingParams.totalFlag} onChange={() => setSettingParams(params => ({ ...params, totalFlag: true }))} />&nbsp;启用
                        </label>
                        <label className='cursor-pointer'>
                            <input type='radio' name='totalFlag' className='cursor-pointer'
                                disabled={settingParams.isDate || !settingParams.type}
                                checked={!settingParams.totalFlag} onChange={() => setSettingParams(params => ({ ...params, totalFlag: false }))} />&nbsp;关闭
                        </label>
                    </div>
                    ...
                    <div className={`flex items-start gap-2 ${!settingParams.totalFlag || settingParams.isDate || !settingParams.type ? 'text-gray-300' : ''}`}>
                        ...
                        >
                            ...
                            <ul className={`absolute right-[8%] top-[100%] text-white text-center bg-sky-500 px-2 overflow-hidden transition-[max-height_.2s_linear] ${flag ? 'max-h-[128px]' : 'max-h-0'}`}>
                                <li className='py-1 hover:text-sky-200' onClick={() => setSettingParams(old => ({ ...old, totalUnit: 0 }))}>天</li>
                                <li className='py-1 hover:text-sky-200' onClick={() => setSettingParams(old => ({ ...old, totalUnit: 1 }))}>小时</li>
                                <li className='py-1 hover:text-sky-200' onClick={() => setSettingParams(old => ({ ...old, totalUnit: 2 }))}>分钟</li>
                                <li className='py-1 hover:text-sky-200' onClick={() => setSettingParams(old => ({ ...old, totalUnit: 3 }))}>秒</li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <button className='w-[88px] px-6 h-[38px] bg-sky-500 text-white rounded-[8px] absolute bottom-3 right-4' onClick={() => {
                        // if (!settingParams.type && settingParams.fixedValue.trim() === '') {
                        //     setSettingParams(params => ({ ...params, fixedNull: true }))
                        //     document.getElementById('fixedValue').focus()
                        // } else if (settingParams.keyName.trim() === '') {
